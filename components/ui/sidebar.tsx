...
function SidebarTrigger({
  ...
  return (
    ...
    >
      <PanelRightIcon />
      {/* ⚠️ عربي: قارئ الشاشة في واجهة عربية يقرأ العنوان كما هو */}
      <span className="sr-only">طيّ الشريط الجانبي</span>
    </Button>
  )
}
...
function SidebarMenuSkeleton({
  ...
  /*
   * ⚠️ **عرض حتميّ لا عشوائي.**
   * النسخة الأصلية تستدعي `Math.random()` أثناء التصيير. و`useMemo` لا
   * يضمن شيئاً — React يجوز أن يُسقط الذاكرة ويُعيد الحساب، فيتغيّر عرض
   * الهيكل بلا سبب. والأسوأ: يختلف بين الخادم والعميل فيُنتج **عدم
   * تطابق ترطيب**.
   *
   * والعرض الثابت كافٍ: تنويع عرض الهياكل زخرفةٌ لا معلومة، وثمنها
   * هنا عدم استقرار وعدم تطابق ترطيب.
   */
  const width = "70%"
