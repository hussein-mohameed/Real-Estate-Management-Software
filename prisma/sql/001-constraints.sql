...
-- ───────────────────────────────────────────────────────────────────────
-- S7 — لوحة واحدة نشطة في المجمَّع
--
-- 🔴 **كان موعوداً في تعليق `Vehicle.plateNumber` ولم يوجد.** المخطّط
--    يقول حرفياً «الفهرس فريد **جزئي** WHERE status <> 'REMOVED' في
--    migration خام» — ولم يُكتب. فبقيت اللوحة قابلة للتسجيل مرّتين.
--
--    وذلك يهمّ الآن تحديداً: تسجيل المركبة صار **ذاتيّاً**. وساكنان
--    يسجّلان نفس اللوحة يُنتجان صفّين، تُوافق الإدارة عليهما، فتصير
--    البوّابة أمام سيارةٍ لها بيتان — ولا تعرف أيّهما تفتح له.
--
-- ⚠️ و`REMOVED` مستثناة عمداً: سيارةٌ بيعت لساكن آخر يجب أن تُسجَّل باسمه.
--    والفريد الكامل كان سيمنع ذلك أبداً.
--
-- ⚠️ و`REJECTED` **داخلةٌ في المنع**: لوحةٌ رُفضت لسبب (ليست له، أو
--    مكرّرة) تُعاد بتسجيلٍ جديد فتلتفّ على الرفض. وإعادة التسجيل تمرّ
--    بالإدارة: تحوّل المرفوضة إلى REMOVED ثم تُسجَّل من جديد.
-- ───────────────────────────────────────────────────────────────────────
CREATE UNIQUE INDEX IF NOT EXISTS "uniq_active_plate"
  ON "Vehicle" ("plateNumber")
  WHERE "status" <> 'REMOVED';
