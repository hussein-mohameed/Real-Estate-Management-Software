...
export default async function RequestDetailPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/*
        ── 🔴 يُقال **لماذا** لا أزرار، ولا تُخفى بصمت ───────────────────
        المالك `R` على الطلبات (‏D3/2: يشرف ولا يشغّل)، فالإسناد وتغيير
        الحالة محجوبان عنه بحقّ. لكن الأزرار كانت تختفي بلا كلمة — فيُقرأ
        الغياب **عطلاً** لا قاعدة، ويُبحَث عن سببٍ في الكود.

        ⚠️ وهذا نفس ما عولج في لوحة التقارير: صلاحيةٌ ناقصة تُقال، ولا
        يُترك المستخدم يستنتجها من فراغ.
      */}
      {!canAct && isOpen ? (
        <p className="rounded-xl bg-muted p-4 text-theme-sm text-muted-foreground">
          {me.role === "OWNER"
            ? "أنت داخلٌ بحساب المالك — وهو يقرأ العمليات ولا يشغّلها (‏D3/2). الإسناد وتغيير الحالة من صلاحية الأدمن."
            : "لا تملك صلاحية تحريك هذا الطلب."}
        </p>
      ) : null}
